...
var USBMailAction = {};
var EXPORTED_SYMBOLS = ['USBMailAction'];
...
USBMailAction.Util = {
  ...
  VersionProxyRunning: false,
	...
	ADDON_ID: "usbmailaction@bohne-lang.de",
	_prefs: null,
  ...
  get Properties() {
    if (!this._properties)
      this._properties = this.StringBundleSvc.createBundle("chrome://usbmailaction/locale/usbmailaction.properties")
    ...
    const Ci = Components.interfaces,
		...
		this._prefs = prefs.getBranch("extensions.usbmailaction.");
		return this._prefs;
	},
  ...
  getTabInfoLength: function getTabInfoLength(tabmail) {
	...
	openHelpTab: function USBMailAction_openHelpTab(fragment) {
		let f = (fragment ? "#" + fragment : ""),
		    URL = "https://github.com/bohnelang/usbmailaction" + f;
		util.getMail3PaneWindow.window.setTimeout(function() {
			USBMailAction.Util.openLinkInTab(URL);
			});
	} ,

	openLinkInTab : function USBMailAction_openLinkInTab(URL) {
		const util = USBMailAction.Util;
				...
				case "Thunderbird":
					...
					if (tabmail) {
						if (!util.findMailTab(tabmail, URL)) {
							...
							{ contentPage: URL}); // , clickHandler: "specialTabs.siteClickHandler(event, USBMailAction_TabURIregexp._thunderbirdRegExp);"
						}
					}
					else {
						util.getMail3PaneWindow.window.openDialog("chrome://messenger/content/", "_blank",
							...
							{ tabType: "contentTab",
								tabParams: {contentPage: URL, id:"USBMailAction_Weblink"}   // , clickHandler: "specialTabs.siteClickHandler(event, USBMailAction_TabURIregexp._thunderbirdRegExp);",
							}
						);
					}
			}
    ...
    try {
      ...
      if (util.Application==='SeaMonkey') {
        ...
        if (browserWin) {
          ...
          this.getMail3PaneWindow.window.openDialog(getBrowserURL(), "_blank", "all,dialog=no", linkURI, null, 'USBMailAction');
        }
  ...
  logToConsole: function logToConsole(msg, optionTag) {
    ...
    consoleService.logStringMessage("USBMailAction "
  ...
  * @msg {string}: text to log
  ...
  getBundleString: function getBundleString(id, defaultText) { 
    ...
    const util = USBMailAction.Util,
      ...
      if (!win) {
          ...
          return;
			...
			if (Cu.import) {
				...
				let versionCallback = function(addon) {
					...
					util.logDebug("AddonManager: USBMailAction extension's version is " + addon.version);
					util.logDebugOptional("firstrun", "USBMailAction.VersionProxy() - DETECTED USBMailAction Version " + util.mExtensionVer);
        ...
        AddonManager.getAddonByID(addonId).then(function(addonId) { versionCallback(addonId); } ); // this function is now a promise
			...
			util.logToConsole("USBMailAction VersionProxy failed - are you using an old version of " + util.Application + "?"
				+ "\n" + ex);
		}
		finally {
	...
	get Version() {
    const util = USBMailAction.Util;
		// returns the current USBMailAction (full) version number.
...
//(function fq_firstRun()
{
  ...
  USBMailAction.Util.FirstRun = {
    init: function init() {
      const prefBranchString = "extensions.usbmailaction.",
      ...
      let prev = -1, 
          firstrun = true, 
          showFirsts = true,     // set false - use this to disable any usbmailaction tabs
          debugFirstRun = false;
      ...
      util.logDebugOptional("firstrun", "Current USBMailAction Version: " + current);
      ...
      try {
        ...
        util.alert("USBMailAction exception in usbmailaction-util.js: " + e.message
          + "\n\ncurrent: " + current
          + "\nprev: " + prev
          + "\nfirstrun: " + firstrun
          + "\ndebugFirstRun: " + debugFirstRun);
        ...
        if (firstrun){  // FIRST TIME INSTALL
          ...
          if (showFirsts) {
            ...
            util.logDebugOptional ("firstrun","setTimeout for content tab (usbmailaction.html)");
            util.getMail3PaneWindow.window.setTimeout(function() {
              util.openLinkInTab("https://github.com/bohnelang/usbmailaction");
            }, 1500); 
          }
        }
        else { 
          ...
          let versionPage = "https://github.com/bohnelang/usbmailaction#" + pureVersion;
